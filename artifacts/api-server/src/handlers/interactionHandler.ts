...
// Modals
import panelEmbedModal from "../modals/panelEmbedModal.js";
...
const buttons = [
  ...
  tradeSetup, paymentProof, openTicket,
  verifyProof, rejectProof, giveRole,
];
...
export async function loadButtons(client: BotClient) {
  for (const btn of buttons) {
    // Store with string key — regex-based ones stored as their source string
    const key = btn.customId instanceof RegExp ? btn.customId.source : btn.customId;
    client.buttons.set(key, btn as any);
    botLog("info", `Loaded button: ${key}`);
  }
}

export async function loadModals(client: BotClient) {
  for (const modal of modals) {
    client.modals.set(modal.customId as string, modal as any);
...
export async function loadSelectMenus(client: BotClient) {
  for (const menu of selectMenus) {
    client.selectMenus.set(menu.customId as string, menu as any);
